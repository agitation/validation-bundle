...
(function(){
var
    validationError = ag.vld.ValidationError,
    ...
    validation =
    {
        ...
        integer : function(value, min, max)
        {
            ...
            if (min !== undefined && value < min)
                throw new validationError(ag.u.sprintf(ag.intl.t("The value is too low, it must be higher than %s."), min));

            if (max !== undefined && value > max)
                throw new validationError(ag.u.sprintf(ag.intl.t("The value is too high, it must be lower than %s."), max));
        },
        ...
        string : function(value, minLength, maxLength)
        {
            ...
            if (!isNaN(minLength) && value.length < minLength)
                throw new validationError(ag.u.sprintf(ag.intl.t("The value is too short, it must have at least %s characters."), minLength));

            if (!isNaN(maxLength) && value.length > maxLength)
                throw new validationError(ag.u.sprintf(ag.intl.t("The value is too long, it must have at most %s characters."), maxLength));
        },
            ...
            if (requiredProperties.length)
            {
                ...
                requiredProperties.forEach(function(property){
                    if (keys.indexOf(property) === -1)
                        throw new validationError(ag.u.sprintf(ag.intl.t("The object is missing the mandatory “%s” property."), property));
                });
            }
        },
    ...
    ag.vld.isValid = function(type /*, ...values */)
    {
        var valid = false;
    ...
    ag.vld.validate = function(type /*, ...values */)
    {
        validation[type].apply(null, getArgs(arguments, 1));
    };

    ag.vld.validateField = function(fieldName, type /*, ...values */)
    {
        try
        {
        ...
        {
            throw new validationError(ag.u.sprintf(ag.intl.t("Error in field “%s”: %s"), fieldName, e.message));
        }
    };
})();
